...
<div class="container">
        ...
        <div class="form-group col-md-3">
				...
				<option value="" selected>----</option>
					@foreach($makes as $make)
						<option value="{{$make->id_Make}}" {{(collect(old('makeID'))->contains($make->id_Make)) ? 'selected':''}}>{{$make->Name}}</option>
					@endforeach
				</select>
		</div>
        <div class="form-group col-md-3">
				...
				@foreach($models as $model)
					<option value="{{$model->id_Model}}" {{(collect(old('modelID'))->contains($model->id_Model)) ? 'selected':''}}>{{$model->Name}}</option>
				@endforeach
				</select>
		</div>
        <div class="form-group col-md-3">
				...
				@foreach($motoTypes as $mototype)
					<option value="{{$mototype->id_MotoType}}" {{(collect(old('motoTypeId'))->contains($mototype->id_MotoType)) ? 'selected':''}}>{{$mototype->Name}}</option>
				@endforeach
				</select>
		</div>
    </div>
    <div class="form-row">
        ...
        <div class="form-group col-md-3">
                ...
                <input type="text" class="form-control" id="engineCapacity" name="engineCapacity" value="{{old('engineCapacity')}}">
		</div>
        <div class="form-group col-md-3">
            ...
            <select class="form-control" id="defect" name="defect">
				...
				@foreach($defects as $defect)
					<option value="{{$defect->id_Defect}}" {{(collect(old('defect'))->contains($defect->id_Defect)) ? 'selected':''}}>{{$defect->Name}}</option>
				@endforeach
				</select>
		</div>
        <div class="form-group col-md-3">
    ...
    <div class="form-row">
        ...
        <div class="form-group col-md-3">
				<label>Rida, km</label>
        <input type="text" class="form-control" id="distanceTraveled" name="distanceTraveled" value="{{old('distanceTraveled')}}">
		</div>
        <div class="form-group col-md-3">
				<label>Spalva</label>
				...
				@foreach($colors as $color)
					<option value="{{$color->id_Color}}" {{(collect(old('color'))->contains($color->id_Color)) ? 'selected':''}}>{{$color->Name}}</option>
				@endforeach
				</select>
		</div>
        <div class="form-group col-md-3">
				<label>Padangų likutis, %</label>
        <input type="text" class="form-control" id="tyreLeft" name="tyreLeft" value="{{old('tyreLeft')}}">
		</div>
    </div>
    <div class="form-row">
        <div class="form-group col-md-3">
        ...
        <div class="form-group col-md-3">
			...
			<select class="form-control" id="coolingType" name="coolingType">
                ...
                @foreach($coolingTypes as $coolingType)
                    <option value="{{$coolingType->id_CoolingTypes}}" {{(collect(old('coolingType'))->contains($coolingType->id_CoolingTypes)) ? 'selected':''}}>{{$coolingType->Name}}</option>
                @endforeach
			</select>
		</div>
        <div class="form-group col-md-3">
			...
			<select class="form-control" id="fuelType" name="fuelType">
                ...
                @foreach($fuelTypes as $fuelType)
                    <option value="{{$fuelType->id_FuelTypes}}" {{(collect(old('fuelType'))->contains($fuelType->id_FuelTypes)) ? 'selected':''}}>{{$fuelType->Name}}</option>
                @endforeach
			</select>
		</div>
        <div class="form-group col-md-3">
			...
			<select class="form-control" id="engineType" name="engineType">
                ...
                @foreach($engineTypes as $engineType)
                    <option value="{{$engineType->id_EngineTypes}}" {{(collect(old('engineType'))->contains($engineType->id_EngineTypes)) ? 'selected':''}}>{{$engineType->Name}}</option>
                @endforeach
			</select>
		</div>
    </div>
	...
	@for($i = 0; $i < sizeof($features); $i++)
		...
		@endif
  			<div class="col-md-3 mt-4">
                ...
                <input type="checkbox" class="form-check-input" id="exampleCheck1" name="additions[]" value="{{$features[$i]["id_Feature"]}}" {{collect(old('additions'))->contains($features[$i]["id_Feature"]) ? 'checked':''}}>
    ...
    <div class="form-row">
        ...
        <div class="form-group col-md-3">
                Tel. Nr.
        <input type="text" class="form-control" name="contactphone" value="{{old('contactphone')}}">
        </div>
        <div class="form-group col-md-3">
				Miestas
				...
				<option value="" selected>----</option>
					@foreach($towns as $town)
						<option value="{{$town->id_Towns}}" {{(collect(old('contacttown'))->contains($town->id_Towns)) ? 'selected':''}}>{{$town->Name}}</option>
					@endforeach
				</select>
		</div>
    </div>
